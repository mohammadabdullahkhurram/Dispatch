...
Dispatch does **not** initiate calls. Voice happens two ways:

- **Inbound IVR calls** hit the `ghl-call` webhook (see
  [docs/ivr-setup.md](./ivr-setup.md)): a ticket is created and the
  call's recording/duration land in the new support session as a
  `call_log` message.
- **Live conversations** use **Google Meet**: the Meet icon (or
  `/meet`) in any chat posts a "Join Google Meet" card that opens in a
  new tab.
